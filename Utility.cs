...
namespace MatchZy
{
    public partial class MatchZy
    {
        ...
        private void SendUnreadyPlayersMessage() {
            ...
            if (isWarmup && !matchStarted)
            {
                ...
                if (unreadyPlayers.Count > 0) {
                    ...
                    Server.PrintToChatAll($"{chatPrefix}: Тоглолтыг эхлүүлэхэд шаардлагатай тоглогчдын тоо: {ChatColors.Green}{minimumReadyRequired}{ChatColors.Default}, бэлэн тоглогчдын тоо: {ChatColors.Green}{countOfReadyPlayers}");
                }
            }
        }
        ...
        public void RoundStartMessage()
        {
            ...
            {
                Server.PrintToChatAll($" [{ChatColors.Green}1sT{ChatColors.Default}] {ChatColors.Green}GL HF!");
                Server.PrintToChatAll($" [{ChatColors.Green}1sT{ChatColors.Default}] {ChatColors.Green}GL HF!");
                Server.PrintToChatAll($" [{ChatColors.Green}1sT{ChatColors.Default}] {ChatColors.Green}GL HF!");
            }
        }
        ...
        private void ForceUnpauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            if (isMatchLive && isPaused)
            {
                ...
                Server.PrintToChatAll($" {ChatColors.Green}ADMIN{ChatColors.Default} тоглолтыг эхлүүллээ, тоглолт үргэлжлэх болно!");
                UnpauseMatch();
        ...
        private void StartMatchMode()
        {
            ...
            ExecUnpracCommands();
            ResetMatch();
            Server.PrintToChatAll($" {ChatColors.Green}1sT PUG mode loaded!");
        }

        private void SendPlayerNotAdminMessage(CCSPlayerController? player) {
            ReplyToUserCommand(player, "Та энэ тушаалыг ашиглах эрхгүй! ^_-");
        }
        ...
        private void SendAvailableCommandsMessage(CCSPlayerController? player)
        {
            ...
            if (matchStarted)
            {
                ...
                ReplyToUserCommand(player, $" Боломжтой тушаалууд:");
